...
(deftest app-router-test
  (testing "Enumerated routes"
   (is (= ["/" "/annotation" "/graph" "/about" "/review" "/analogy"
  ...
  (testing "Project routes"
   (is (= {:id "123"}
          (:path-params (r/match-by-path (sut/app-router)
                                         "/project/doc/123"))))))
   ...
   (is
    (=
     ...
     (-> {:request-method :get
         ...
         (update-in [:docs] (comp vec (partial sort-by :id)))
         (update-in [:docs 4] dissoc :content)
         (update :graphs (partial map #(update % :edges (partial sort-by :id))))
