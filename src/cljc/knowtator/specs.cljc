...
(s/def ::hex-color
  (s/spec (s/and string? (partial re-matches hex-color-re))
          :gen #(sg/string-generator hex-color-re)))

(s/def ::color
  (s/or :hex        ::hex-color
        :html       html-colors/html-colors
        :lower-html (->> html-colors/html-colors
                         keys
                         (map str/lower-case)
                         set)))
...
(s/def :span-overlap/span
  (s/keys :req-un [:span/start :span/end :span-overlap/id :span-overlap/ann]))
...
(s/def ::content string?)
(s/def ::spans-with-spanned-text
  (s/keys :req-un [::id :span/ann :span/start :span/end ::content]))
