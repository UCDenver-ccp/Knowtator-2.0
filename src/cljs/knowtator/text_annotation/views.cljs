                                 ...
                                 [re-com/p-span
                                   {:id    e-id
                                    :style (cond-> {:background-color color
                                                    ...
                                                    :border-color     color
                                                    :cursor           :pointer}
                                             (coll? id) (assoc :border-color
                                                               :grey)
                                             selected?  (assoc :border-color
                                                               :black)
                                             #_#_searched (assoc :color :red))}
                                   content])))
      :component-did-mount (fn [_]
                             (let [e (.getElementById js/document e-id)]
                               (when (and (<sub [::subs/un-searched?])
                                          #_searched)
...
(defn editor-paragraph
  [paragraph]
  [re-com/p {:style {:text-align   :justify
                     :text-justify :inter-word}}
    (doall (for [text paragraph]
             (if (string? text)
               (if (empty? text) " " text)
               ^{:key (str (random-uuid))} [popup-text-annotation text])))])
  ...
  [re-com/v-box
    ...
    :children [[re-com/title
               ...
               [re-com/scroller
                 :width "min-content"
                 :child [dt/datatable
                          ::annotation-id [::subs/selected-ann-info]
                          [{::dt/column-key   [:prop]
