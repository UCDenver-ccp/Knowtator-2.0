     ...
     [[:img
       (let [size "60px"]
         {:src   "https://avatars.githubusercontent.com/u/1854424?s=200&v=4"
          :style {:width  size
                  :height size}})]
      [re-com/title :label "Knowtator" :level :level1]]]
    #_[re-com/h-box
       :children
       [[re-com/single-dropdown
        ...
        [re-com/button
         ...
         "Open project"
         :on-click
         #(>evt [::evts/load-project (<sub [::subs/selected-project])])]
        [re-com/alert-list
         :on-close
         #(println %)
         :alerts
...
(defn undo-controls
  ...
  [re-com/h-box
   :children
   [[re-com/md-circle-icon-button :md-icon-name "zmdi-undo" :on-click
     #(>evt [:undo])]
    [re-com/md-circle-icon-button :md-icon-name "zmdi-redo" :on-click
     #(>evt [:redo])]]])
...
(defn search-controls
  ...
  [re-com/h-box
   ...
   [[re-com/input-text
     :model
     (<sub [::subs/search-query])
     :on-change
     #(>evt [::evts/update-search-text %])
     ...
     :placeholder
     "search document"]
    [re-com/label :label (<sub [::subs/search-matches])]]])
  ...
  [re-com/v-box
   ...
   [[tav/doc-header (<sub [::subs/selected-doc])]
    [re-com/h-split
     ...
     85
     :panel-1
     [tav/editor (<sub [::subs/selected-doc])]
     :panel-2
     [tav/annotation-info]]]])
...
(defn annotation-panel
  ...
  [re-com/v-box
   ...
   :children
   [[undo-controls]
    [re-com/h-box :children [[tac/profile-controls] [tac/doc-controls]]]
    [re-com/h-box :children [[color-controls] [tac/ann-controls]]]
    [re-com/h-split
     :src
     (at)
     :initial-split
     ...
     :panel-2
     [doc-display]]
    [re-com/h-box :children [[tac/span-controls] [search-controls]]]]])
  ...
  (let [showing? (r/atom false)]
    [:div
     ...
     [re-com/popover-anchor-wrapper
      ...
      [re-com/button
       :on-click
       (re-com/handler-fn (swap! showing? not))
       :label
       "Filters"]
      :popover
...
(defn review-panel
  []
  [:div
   [re-com/title :src (at) :label "Review" :level :level1]
   [review/chooser]
   [review/table]
   [filter-controls]])
   ...
   [[ra-controls/graph-space-controls
    ...
    [re-com/h-box
     ...
     [(map (fn [e] (with-meta e {:key (str (random-uuid))}))
           (interpose [re-com/line :size "4px" :color "black"]
...
(defn main-panel
  ...
  (let [active-panel (rf/subscribe [::subs/active-panel])]
    [re-com/v-box
     ...
     "100%"
     :children
     [[page-title] (routes/panels @active-panel)]]))
