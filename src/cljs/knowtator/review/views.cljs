     ...
     [{::dt/column-key   [:filter-type]
      ...
      {::dt/column-key   [:item]
       ...
       ::dt/render-fn    (fn [val {:keys [filter-type]}]
                           [re-com/input-text
                            :model     (str (name val))
                            :on-change #(>evt [::evts/update-review-filter-item
                                               val %
                                               filter-type])])}]
     ...
     (when cols
       [dt/datatable
        ...
        [::subs/values-to-review
         [::dt/selected-items ::filter-table [::subs/flattened-review-filters]]]
        (for [col cols]
         ...
         (fn [] [:tr
                 [:th {:col-span 6}
                  [:strong "Total selected: "
                   (count
                    (<sub [::dt/selected-items ::text-annotation-table
                           [::subs/values-to-review
                            [::dt/selected-items ::filter-table
