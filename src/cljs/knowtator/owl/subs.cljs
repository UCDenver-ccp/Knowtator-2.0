...
(reg-sub ::ann-props (comp :ann-props :ontology))

(reg-sub ::classes (comp :classes :ontology))

(reg-sub ::selected-concept (fn [db _] (get-in db [:selection :concepts])))

(reg-sub ::selected-obj-prop (fn [db _] (get-in db [:selection :obj-props])))
...
(reg-sub ::obj-prop-uri->label
  :<- [::obj-props]
  :<- [::selected-ann-prop]
  (fn [[obj-props ann-prop] _] (make-uri->label-map obj-props ann-prop)))

(reg-sub ::classes-uri->label
  :<- [::classes]
  :<- [::selected-ann-prop]
  (fn [[classes ann-prop] _] (make-uri->label-map classes ann-prop)))

(reg-sub ::class-hierarchy-zippers
  :<- [::class-hierarchy]
  (fn [h _] (h/hierarchy-zippers identity identity h)))
...
(reg-sub ::class-map
  ...
  (fn [classes _]
    (->> classes
         (util/map-with-key (comp (partial apply str)
...
(reg-sub ::owl-class-label
  :<- [::classes-uri->label]
  (fn [class-map [_ iri]] (get class-map iri)))

(reg-sub ::owl-obj-prop-label
  :<- [::obj-prop-uri->label]
  (fn [obj-prop-map [_ iri]] (get obj-prop-map iri)))

(reg-sub ::selected-ann-prop (fn [db _] (get-in db [:selection :ann-props])))


(reg-sub ::selected-owl-hierarchy
  (fn [db _] (get-in db [:selection :owl-hierarchy] :owl-class-hierarchy)))
