...
(reg-event-db ::set-ontology
  (fn [db [_ result]]
    (-> db
      ...
      (assoc-in [:selection :ann-props] (->> result :ann-props first map->iri))
      (assoc-in [:selection :concepts] (->> result :classes first map->iri))
      (assoc-in [:selection :obj-props] (->> result :obj-props first map->iri)))))

(defn select-hierarchy-node [db k h iri]
  (-> db
...
(defn toggle-collapse [db k iri]
  (update-in db [:ontology k]
    (fn [ms]
      (-> ms
...
(reg-event-db ::toggle-collapse-owl-class
  (fn [db [_ iri]]
    (toggle-collapse db :classes iri)))
