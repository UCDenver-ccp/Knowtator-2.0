...
public class KnowtatorView extends AbstractOWLClassViewComponent implements DropTargetListener {
    ...
    private void setListeners() {
        controller.addProjectListener(textPane);
    }
...
//        log.warn("Initializing class view");
        ...
        spanList = new SpanList(controller);
...
//        log.warn("Creating UI components");
        ...
        textPane = new TextPane(controller);
    ...
    private void makeButtons() {
            ...
            if (owlClass != null) {
                ...
                if (c != null) {
                    ...
                    if (JOptionPane.showConfirmDialog(controller.getView(), "Assign color to descendants of " + owlClass + "?") == JOptionPane.OK_OPTION) {
                        Set<OWLClass> descendants = controller.getOWLAPIDataExtractor().getDescendants(owlClass);
                        if (descendants != null) {
        ...
        growSelectionStartButton.addActionListener((ActionEvent e) -> {
            if (controller.getSelectionManager().getSelectedSpan() == null) {
                textPane.growStart();
            } else {
                controller.getSelectionManager().getActiveTextSource().getAnnotationManager().growSelectedSpanStart();
            }
        });
        shrinkSelectionStartButton.addActionListener((ActionEvent e) -> {
            if (controller.getSelectionManager().getSelectedSpan() == null) {
                textPane.shrinkStart();
            } else {
                controller.getSelectionManager().getActiveTextSource().getAnnotationManager().shrinkSelectedSpanStart();
            }
        });
        shrinkSelectionEndButton.addActionListener((ActionEvent e) -> {
            if (controller.getSelectionManager().getSelectedSpan() == null) {
                textPane.shrinkEnd();
            } else {
                controller.getSelectionManager().getActiveTextSource().getAnnotationManager().shrinkSelectedSpanEnd();
            }
        });
        growSelectionEndButton.addActionListener((ActionEvent e) -> {
            if (controller.getSelectionManager().getSelectedSpan() == null) {
                textPane.growEnd();
            } else {
                controller.getSelectionManager().getActiveTextSource().getAnnotationManager().growSelectedSpanEnd();
            }
        });

        previousSpanButton.addActionListener((ActionEvent e) -> controller.getSelectionManager().getActiveTextSource().getAnnotationManager().getPreviousSpan());
        nextSpanButton.addActionListener((ActionEvent e) -> controller.getSelectionManager().getActiveTextSource().getAnnotationManager().getNextSpan());
        ...
        removeAnnotationButton.addActionListener(e -> {
            if (JOptionPane.showConfirmDialog(controller.getView(), "Are you sure you want to remove the selected annotation?", "Remove Annotation", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                controller.getSelectionManager().getActiveTextSource().getAnnotationManager().removeSelectedAnnotation();
            }
        });
        addAnnotationButton.addActionListener(e -> controller.getSelectionManager().getActiveTextSource().getAnnotationManager().addSelectedAnnotation());

        previousTextSourceButton.addActionListener(e -> controller.getTextSourceManager().getPreviousTextSource());
        nextTextSourceButton.addActionListener(e -> controller.getTextSourceManager().getNextTextSource());

        decreaseFontSizeButton.addActionListener((ActionEvent e) -> textPane.decreaseFontSize());
        increaseFontSizeButton.addActionListener((ActionEvent e) -> textPane.increaseFindSize());
        textSourceChooser.addActionListener(e -> {
            JComboBox comboBox = (JComboBox) e.getSource();
            if (comboBox.getSelectedItem() != null) {
                controller.getSelectionManager().setActiveTextSource((TextSource) comboBox.getSelectedItem());
            }
        });
        profileChooser.addActionListener(e -> {
            JComboBox comboBox = (JComboBox) e.getSource();
            if (comboBox.getSelectedItem() != null) {
                controller.getSelectionManager().setActiveProfile((Profile) comboBox.getSelectedItem());
            }
        });
        nextMatchButton.addActionListener(e -> {
            String textToFind = matchTextField.getText();
            TextPane currentTextPane = controller.getView().getTextPane();
    ...
    private void $$$setupUI$$$() {
        ...
        textPane.setForeground(new Color(-16777216));
        textPane.setText("");
