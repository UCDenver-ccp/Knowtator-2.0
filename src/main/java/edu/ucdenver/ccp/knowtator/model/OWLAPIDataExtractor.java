...
public class OWLAPIDataExtractor implements Savable {
		...
		try {
			annotationValue =
					EntitySearcher.getAnnotations(
							ent, owlWorkSpace.getOWLModelManager().getActiveOntology(), annotationProperty)
							.stream()
	...
	OWLClass getOWLClassByID(String classID) {
	...
	public String getOWLClassID(OWLClass cls) {
		if (owlWorkSpace != null && cls != null) {
			String annotationValue =
					extractAnnotation(
							cls, owlWorkSpace.getOWLModelManager().getOWLDataFactory().getRDFSLabel());
