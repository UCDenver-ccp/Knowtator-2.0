...
public class GraphSpace extends mxGraph implements Savable {
   ...
   */
	...
	public void addListener(GraphSpaceListener listener) {
	...
	public void addTriple(
		...
		Triple newTriple =
				new Triple(id, source, target, property, annotator, quantifier, quantifierValue);
							...
							if (edge instanceof Triple) {
								...
								if (value instanceof String) {
									OWLObjectProperty property =
											controller.getOWLAPIDataExtractor().getOWLObjectPropertyByID((String) value);
									if (property != null) {
										((Triple) edge).setValue(property);
									} else {
										log.warn(
												String.format(
														"Edge not found in ontology: %s", ((Triple) edge).getValue()));
									}
								}
							}
						});
